...
//	console.log(id1,id2,id3,id4)
	...
	if(GetUrlString("from") !=null && GetUrlString("from").toString().length>1){
		...
		$(".ljdy_crops_buy").click(function(){
			...
			var tsPrompt_list=$(this).parent().siblings(".tsPrompt_list");			
			//				刷新token
			if(tokenValue==''||tokenValue=='(null)'||!tokenValue){
				refreshtoken();
			}	
				
			if(subscribe_name=='zf'){
				isPaid(id1,tokenValue,tsPrompt_list);
			}else if(subscribe_name=='yf'){				
				isPaid(id2,tokenValue,tsPrompt_list);	
			}else if(subscribe_name=='xh'){
				isPaid(id3,tokenValue,tsPrompt_list);
			}else if(subscribe_name=='pq'){
				isPaid(id4,tokenValue,tsPrompt_list);
			}
		})
...
//		配合ios传参,如果wbversion有值,当点击各个战队时候也要把wbversion传递过去,有wbversion才掉ios分享功能
	...
	function isPaid(refId,token,tsPrompt_list){
		$.ajax({
			type:"get",		
//			url:"http://advisor-dev.0606.com.cn:5510/api/assemble/product?ref_id="+refId+"&access_token="+token,
			url:"http://advisor.0606.com.cn:80/api/assemble/product?ref_id="+refId+"&access_token="+token,
			dataType:'json',
...
//					已购买过为true，提示已经购买过
					if(datas.data[0].is_paid){
						tsPrompt_list.fadeIn();
						setTimeout(function(){
							tsPrompt_list.fadeOut();
						},1500)
					}else{
//						refId就是战队id
						payment(refId,type);/*未购买过跳转支付*/
					}					
...
//              from:如果是分享出去的页面,那么跳转单个战队需要传值为share,否则不传（分享详情页点击订阅时使用）
				window.location.href=HttpLink+"zfzd.html?plan_name=zf&plan_id="+id1+share+"&access_token="+token;
			}else if(plan_name=="yf"){
				window.location.href=HttpLink+"yfzd.html?plan_name=yf&plan_id="+id2+share+"&access_token="+token;
			}else if(plan_name=='xh'){
				window.location.href=HttpLink+"xhzd.html?plan_name=xh&plan_id="+id3+share+"&access_token="+token;
			}else if(plan_name=='pq'){
				window.location.href=HttpLink+"pqzd.html?plan_name=pq&plan_id="+id4+share+"&access_token="+token;
			}
		})
	}
